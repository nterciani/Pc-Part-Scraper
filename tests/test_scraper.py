...
def test_extract_cpu_info() -> None:
    """Test that cpu info is properly extracted."""
    ...
    assert scraper.extract_cpu_info(amd1) == ("AMD Ryzen 9 9950X", "AMD")
    assert scraper.extract_cpu_info(amd2) == ("AMD Ryzen 7 7800X3D", "AMD")

    assert scraper.extract_cpu_info(intel1) == ("Intel Core i5-13400F", "Intel")
    assert scraper.extract_cpu_info(intel2) == ("Intel Core Ultra 9 285K", "Intel")

    assert scraper.extract_cpu_info(threadripper) == ("AMD Ryzen Threadripper PRO 7965WX", "AMD")
    assert scraper.extract_cpu_info(pentium) == ("Intel Pentium G7400", "Intel")
